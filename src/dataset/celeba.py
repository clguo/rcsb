...
class CelebADataset(Dataset):
    """
    torch.utils.data.Dataset for CelebA. Assumes that images are stored as:
    ...
    def get_paths(self, path):
        paths = [Path(path) / "imgs" / str(idx) / "img.png" for idx in range(202_599)]
        return paths
    ...
    def __getitem__(self, index):
        ...
        if pred_label != -1: log.info(f"Initial prediction: {pred_label.item()}")
